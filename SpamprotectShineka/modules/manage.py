...
@SpamprotectShineka.on_message(filters.command("nsfw_scan"), group=3)
async def nsfw_scan_command(_, message: Message):
    ...
    file = await SpamprotectShineka.download_media(file_id)
    try:
...
@SpamprotectShineka.on_message(filters.command("spam_scan"), group=3)
